    ...
    <section class="hero">
        <div class="hero-grid">
            <div>
                ...
                <h1>Triagem morfológica com YOLOv8 + DenseNet</h1>
                <p>
                    Uma interface integrada para detecção e classificação avançada
                    de neutrófilos segmentados e bastonetes em microfotografias de esfregaços sanguíneos.
                </p>
                <div class="hero-actions">
                    <span class="hero-chip">YOLOv8 + DenseNet121</span>
            ...
            <div class="signal-panel">
                <div class="signal-panel-header">
                    <strong>Fluxo de análise (Two-Stage)</strong>
                    <span class="signal-status">ativo</span>
                </div>
                <div class="signal-list">
                    ...
                    <div class="signal-item">
                        <span>Etapa 2</span>
                        <b>DenseNet121 (Classificação)</b>
                    </div>
                    <div class="signal-item">
                        <span>Saída</span>
                        <b>Relatório e métricas</b>
                    </div>
                </div>
            </div>
        </div>
...
st.markdown(
    ...
    <div class="metric-grid">
        <div class="metric-card">
            ...
            <span>Localização de células por visão computacional.</span>
        </div>
        <div class="metric-card">
            <strong>Classificação fina</strong>
            <span>Análise morfológica detalhada via DenseNet.</span>
        </div>
        <div class="metric-card">
            <strong>Resultado exportável</strong>
            <span>Resumo em CSV para documentação acadêmica.</span>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
def load_densenet():
    ...
    if os.path.exists(model_path) and os.path.getsize(model_path) > 1024 * 1024:
        ...
        for key in cleaned_state_dict.keys():
            if "classifier.weight" in key or "fc.weight" in key or "linear.weight" in key:
...
model_densenet, num_classes_detectado = load_densenet()
...
# Rótulos reais exatos das 6 classes do modelo DenseNet treinado para o TCC
classes_densenet = [
    "Bastonete",
    "Eosinófilo",
    "Linfócito",
...
if file:
    ...
    with st.spinner("Analisando com YOLOv8 e classificando com DenseNet..."):
        results = model_yolo.predict(img, conf=conf_threshold)
    ...
    if boxes and len(boxes) > 0:
        for box in boxes:
            ...
            cropped_cell = img.crop((x1, y1, x2, y2))
            tensor_img = transform_densenet(cropped_cell).unsqueeze(0)
            
            with torch.no_grad():
                outputs = model_densenet(tensor_img)
                probabilities = torch.nn.functional.softmax(outputs, dim=1)
                dn_conf, pred_idx = torch.max(probabilities, 1)
                
            idx = pred_idx.item()
            class_name = classes_densenet[idx] if idx < len(classes_densenet) else f"Classe {idx}"
            detections_classes.append(class_name)
            
            draw.rectangle([x1, y1, x2, y2], outline="#ff2a2a", width=4)
            label_text = f"{class_name} ({dn_conf.item()*100:.1f}%)"
            draw.text((x1, max(0, y1-15)), label_text, fill="#ff2a2a")

        col1, col2 = st.columns(2)
        with col1:
            st.image(img, caption="Imagem original", use_container_width=True)
        with col2:
            st.image(img_draw, caption="Imagem analisada (Classificação DenseNet)", use_container_width=True)

        st.markdown('<div class="result-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">Relatório automático (DenseNet Classifier)</div>', unsafe_allow_html=True)

        counts = {}
        for name in sorted(set(detections_classes)):
            quantity = detections_classes.count(name)
        ...
        total_cells = len(detections_classes)
        band_count = sum(qty for name, qty in counts.items() if "bastonete" in name.lower())
        segmented_count = sum(qty for name, qty in counts.items() if "segmentado" in name.lower())
